...
description: Use when creating or updating transactional email templates in MambaSplit.Api — matches the repo's existing email pipeline and visual style rather than inventing a new one. Trigger on requests like "add a new transactional email", "redesign this email template", or "update the copy in the invite email".
